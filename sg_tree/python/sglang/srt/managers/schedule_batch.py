...
class Req:
    ...
    def init_next_round_input(self, tree_cache: Optional[BasePrefixCache] = None):
        ...
        # Tree attention: skip prefix matching entirely
        if self.is_tree_attention:
            self.prefix_indices = torch.empty((0,), dtype=torch.int64)
            self.extend_input_len = input_len
            return
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_for_extend(self):
        ...
        # Build tree attention mask if needed
        self.is_tree_attention = any(r.is_tree_attention for r in reqs)
        if self.is_tree_attention:
            from sglang.srt.managers.tree_attention_utils import (
                build_tree_mask,
                compute_tree_positions,
            )

            parent_ids_list = [r.parent_ids for r in reqs]
            self.tree_custom_mask, self.tree_mask_indptr = build_tree_mask(
                parent_ids_list, self.device
            )
            # Compute tree positions (concatenated for all reqs)
            all_positions = []
            for pid in parent_ids_list:
                all_positions.extend(compute_tree_positions(pid))
            self.tree_positions = torch.tensor(
                all_positions, dtype=torch.int64
            ).to(self.device, non_blocking=True)

            # Build tree logprob parent indices:
            # For each token i (except root), its logprob should come from
            # logits[parent[i]], not logits[i-1]. We build per-request
            # parent indices offset by the cumulative extend start in pruned space.
            tree_logprob_parent_indices = []
            offset = 0
            for req_idx, req in enumerate(reqs):
                if req.is_tree_attention and self.return_logprob:
                    pid = req.parent_ids
                    n = len(pid)
                    logprob_start = req.extend_logprob_start_len
                    # For tokens (logprob_start+1)..N-1, use parent indices
                    for i in range(max(1, logprob_start + 1), n):
                        tree_logprob_parent_indices.append(offset + pid[i] - logprob_start)
                    # Add the sampling position (last token, N-1)
                    tree_logprob_parent_indices.append(offset + n - 1 - logprob_start)
                offset += req.extend_input_len - req.extend_logprob_start_len
            self.tree_logprob_parent_indices = tree_logprob_parent_indices if tree_logprob_parent_indices else None
        else:
            self.tree_custom_mask = None
            self.tree_mask_indptr = None
            self.tree_positions = None
            self.tree_logprob_parent_indices = None
    ...
    def get_model_worker_batch(
        ...
        return ModelWorkerBatch(
            ...
            is_tree_attention=getattr(self, "is_tree_attention", False),
            tree_custom_mask=getattr(self, "tree_custom_mask", None),
            tree_mask_indptr=getattr(self, "tree_mask_indptr", None),
            tree_positions=getattr(self, "tree_positions", None),
            tree_logprob_parent_indices=getattr(self, "tree_logprob_parent_indices", None),
        )
...
class ModelWorkerBatch:
    ...
    # Tree attention
    is_tree_attention: bool = False
    tree_custom_mask: Optional[torch.Tensor] = None
    tree_mask_indptr: Optional[torch.Tensor] = None
