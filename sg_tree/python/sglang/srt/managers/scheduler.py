...
class Scheduler(
    ...
    def handle_generate_request(
        ...
        if (
            ...
            req = Req(
                ...
                parent_ids=recv_req.parent_ids,
            )
            req.tokenizer = self.tokenizer

            # Tree attention: force max_new_tokens=0 (scoring only, no generation)
            if req.is_tree_attention:
    ...
    def _get_new_batch_prefill_raw(
        ...
        for req in self.waiting_queue:
            ...
            # Tree attention requests must not mix with normal requests
            if adder.can_run_list and (
                getattr(req, 'is_tree_attention', False) != getattr(adder.can_run_list[0], 'is_tree_attention', False)
            ):
                break
