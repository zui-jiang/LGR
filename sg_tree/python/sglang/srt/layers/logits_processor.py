...
class LogitsMetadata:
    ...
    # Tree attention: parent indices for logprob computation
    tree_logprob_parent_indices: Optional[list] = None
    ...
    def from_forward_batch(cls, forward_batch: ForwardBatch):
        ...
        tree_lpi = getattr(forward_batch, 'tree_logprob_parent_indices', None)
        ...
        return cls(
            ...
            tree_logprob_parent_indices=tree_lpi,
        )
...
class LogitsProcessor(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            # For tree attention, remap logprob indices to use parent positions
            if logits_metadata.tree_logprob_parent_indices is not None:
                input_logprob_indices = logits_metadata.tree_logprob_parent_indices
        ...
        if should_skip_chunking:
            if logits_metadata.tree_logprob_parent_indices is not None:
                # Tree attention: deduplicate + chunked logits computation.
                # Multiple children share the same parent's logits.
                # We compute logits in chunks to avoid OOM.
                (logprobs_result, sampled_logits) = (
                    self._process_tree_logprobs_chunked(
                        pruned_states,
                        sample_indices,
                        input_logprob_indices,
                        lm_head,
    ...
    def _process_tree_logprobs_chunked(
        self,
        ...
        """
        Tree attention logprobs with deduplication + chunked computation.
        Multiple children share the same parent's logits, so we only compute
        logits for unique positions, and process them in chunks to avoid OOM.
        """
        chunk_size = self.logprobs_chunk_size if self.enable_logprobs_chunk else 512
        device = pruned_states.device

        # 1. Deduplicate: find unique positions needed
        input_indices_list = input_logprob_indices.tolist()
        sample_indices_list = (
            sample_indices.tolist() if sample_indices is not None else []
        )

        all_needed = sorted(set(input_indices_list + sample_indices_list))
        pos_to_uid = {pos: i for i, pos in enumerate(all_needed)}
        n_unique = len(all_needed)

        # Map input positions -> unique position index
        input_to_unique = torch.tensor(
            [pos_to_uid[p] for p in input_indices_list], device=device, dtype=torch.long
        )
        sample_to_unique = torch.tensor(
            [pos_to_uid[s] for s in sample_indices_list], device=device, dtype=torch.long
        ) if sample_indices_list else None

        # 2. Pre-allocate results
        n_input = len(input_indices_list)
        input_token_logprobs = torch.empty(n_input, device=device)
        ...
        if top_k > 0:
            unique_top_vals = torch.empty(n_unique, top_k, device=device)
            unique_top_idxs = torch.empty(
                n_unique, top_k, device=device, dtype=torch.long
            )

        sampled_logits = None

        # 3. Process unique positions in chunks
        unique_positions_t = torch.tensor(all_needed, device=device, dtype=torch.long)

        for chunk_start in range(0, n_unique, chunk_size):
            chunk_end = min(chunk_start + chunk_size, n_unique)
            chunk_pos = unique_positions_t[chunk_start:chunk_end]

            # Compute logits for this chunk only
            chunk_logits = self._get_logits(
                pruned_states[chunk_pos], lm_head, logits_metadata
            )

            # Handle sampled logits
            if sample_to_unique is not None:
                s_mask = (sample_to_unique >= chunk_start) & (
                    sample_to_unique < chunk_end
                )
                if s_mask.any():
                    local_s = sample_to_unique[s_mask] - chunk_start
                    sampled_logits = chunk_logits[local_s]

            # Compute logprobs (log_softmax)
            chunk_logprobs = torch.nn.functional.log_softmax(chunk_logits, dim=-1)
            del chunk_logits

            # Extract token logprobs for input positions in this chunk
            inp_mask = (input_to_unique >= chunk_start) & (
                input_to_unique < chunk_end
            )
            if inp_mask.any():
                inp_indices = torch.nonzero(inp_mask, as_tuple=True)[0]
                local_unique = input_to_unique[inp_indices] - chunk_start
                token_ids = logits_metadata.extend_input_logprob_token_ids_gpu[
                    inp_indices
                ]
                input_token_logprobs[inp_indices] = chunk_logprobs[
                    local_unique, token_ids
                ]

            # Extract top-k per unique position
            if top_k > 0:
                vals, idxs = chunk_logprobs.topk(top_k, dim=-1)
                unique_top_vals[chunk_start:chunk_end] = vals
                unique_top_idxs[chunk_start:chunk_end] = idxs

            del chunk_logprobs

        # 4. Format top-k results to match get_top_logprobs_raw output
        if top_k > 0:
            # Remap from unique positions to input positions
            remapped_vals = unique_top_vals[input_to_unique].tolist()
            remapped_idxs = unique_top_idxs[input_to_unique].tolist()
            del unique_top_vals, unique_top_idxs

            # Single request for tree attention
            k = logits_metadata.top_logprobs_nums[0]
            input_top_logprobs_val = [[v[:k] for v in remapped_vals]]
            input_top_logprobs_idx = [[idx[:k] for idx in remapped_idxs]]
        else:
            input_top_logprobs_val = None
            input_top_logprobs_idx = None
